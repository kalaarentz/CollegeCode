...
public class LatinSquareModel {
	...
	public static void main( String[] args )
	{
		...
		//System.out.println( "Is Complete Overide " 
		//+ newGame.isComplete( newGame.getLatinBoardCopy() ) ); 
	}
			...
			for( int j = 0; j < board[ i ].length; j++ )
			{
				if( board[ i ][ j ] == null )
				{
					...
					for( int idx = 0; idx < validSymbols.length; idx++ )
					{
						...
						if( !checkForDuplicates( i, j, board) )
						{
	...
	private boolean isComplete( String[][] board )
	{
		...
		for( int row = 0; row < board.length; row++ )
		{
			...
			{
				if( checkForDuplicates( row, col, board ) )
				{
					return false;
				}
			}
