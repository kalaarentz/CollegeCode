...
bool CMainApplication::BInit()
{
	...
	m_pCompanionWindow = SDL_CreateWindow( "HW-6", nWindowPosX, nWindowPosY, m_nCompanionWindowWidth, m_nCompanionWindowHeight, unWindowFlags );
	...
	std::string strWindowTitle = "Ex-1 - " + m_strDriver + " " + m_strDisplay;
...
bool CMainApplication::HandleInput()
{
	...
	for( vr::TrackedDeviceIndex_t unDevice = 0; unDevice < vr::k_unMaxTrackedDeviceCount; unDevice++ )
	{
      ...
      // if controller pass to draw code
			if ( !m_pHMD->IsTrackedDeviceConnected( unDevice ) ) continue;
			if( m_pHMD->GetTrackedDeviceClass( unDevice ) != vr::TrackedDeviceClass_Controller ) continue;
			draw_UpdateDeviceState(unDevice,state.ulButtonPressed != 0);
		}
	}
...
void CMainApplication::RenderControllerAxes()
{
	...
	for ( vr::TrackedDeviceIndex_t unTrackedDevice = vr::k_unTrackedDeviceIndex_Hmd + 1; unTrackedDevice < vr::k_unMaxTrackedDeviceCount; ++unTrackedDevice )
	{
		...
		// capture current pose for device
 		// would prefer to obtain this at the start of our draw cycle
     draw_UpdateDevicePose(unTrackedDevice,mat);
...
{
	// allow updates prior to drawing
	draw_Update();
	// do shadow work for scene
  draw_Shadow();
...
void CMainApplication::RenderScene( vr::Hmd_Eye nEye )
{
	...
	if( m_bShowCubes )
	{
		...
		Matrix4 projMat = (nEye == vr::Eye_Left) ? (m_mat4ProjectionLeft) : (m_mat4ProjectionRight);
		Matrix4 viewMat = (nEye == vr::Eye_Left) ? (m_mat4eyePosLeft * m_mat4HMDPose) : (m_mat4eyePosRight * m_mat4HMDPose);
		draw_Scene(GetCurrentProjectionMatrix(nEye),GetCurrentViewMatrix(nEye));
	}
...
int main(int argc, char *argv[])
{
	...
	// perform our initialization, must be after glew is inited by abpve
	draw_Init();
