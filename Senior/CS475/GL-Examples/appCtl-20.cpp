...
struct appAttr app_Attributes() {
  struct appAttr attr = {"Ex 20", true, true};
  return attr;
}
...
void app_LeftMouseMotion(double x, double y) {
  // struct drawable drawable = scene_NextSelected(true);
  // if (drawable.selectTag != -1) {
  //   do {
  //     Vector4 mq = motionQuat(UNITIZE(x,WinWidth),-UNITIZE(y,WinHeight));
  //     Vector4 tbq = tb_Quat();
  //     Vector4 invtbq = Vector4(-tbq[0],-tbq[1],-tbq[2],tbq[3]);
  //     Vector4 t = combineQuats(invtbq,mq);
  //     Vector4 q = combineQuats(t,tbq);
  //     Matrix4 m = tb_MatrixForQuat(q);
  //     fprintf(stderr,"setting\n");
  //     mutil_Print4x4(true,"pre",drawable.transforms);
  //     drawable.transforms = drawable.transforms * m;
  //     mutil_Print4x4(true,"post",drawable.transforms);
  //     drawable = scene_NextSelected(false);
  //   } while (drawable.selectTag != -1);
  // }
  // else {
