...
struct appAttr app_Attributes() {
  struct appAttr attr = {"Ex 6", false, false};
  return attr;
}
...
void app_KeyEvent(int event) {
  switch (event) {
    case 'F':
      state["offsetFactor"] = state["offsetFactor"].get<int>() + 1;
      break;
    case 'f':
      state["offsetFactor"] = state["offsetFactor"].get<int>() - 1;
      break;
    case 'U':
      state["offsetUnits"] = state["offsetUnits"].get<int>() + 1;
      break;
    case 'u':
      state["offsetUnits"] = state["offsetUnits"].get<int>() - 1;
      break;
  ...
  fprintf(stdout,"offset factor %d units %d\n",state["offsetFactor"].get<int>(),state["offsetUnits"].get<int>());
}
