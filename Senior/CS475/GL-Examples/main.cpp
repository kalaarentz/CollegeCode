...
bool trackingMoved = false;
int mouseDownX;
int mouseDownY;
float winWidth = 300.0;
float winHeight = 300.0;

SDL_Window * init(int argc, char *argv[]) {
  ...
  struct appAttr attr = app_Attributes();

  SDL_Window * window = SDL_CreateWindow(attr.title,SDL_WINDOWPOS_CENTERED,SDL_WINDOWPOS_CENTERED,winWidth,winHeight,SDL_WINDOW_OPENGL | SDL_WINDOW_RESIZABLE);
  if (!window) {
    fprintf(stderr,"SDL_CreateWindow failed: %s\n",SDL_GetError());
    return NULL;
  }

  int profile = (attr.coreProfile) ?  (SDL_GL_CONTEXT_PROFILE_CORE) : (SDL_GL_CONTEXT_PROFILE_COMPATIBILITY);
  ...
  app_Init(winWidth,winHeight,argc,argv);
...
//  fprintf(stderr,"%lf,%lf\n",xpos,ypos);
  if (trackingMouse) {
    if (!trackingMoved) app_LeftMouseDown(mouseDownX,mouseDownY);
    app_LeftMouseMotion(xpos,ypos);
    trackingMoved = true;
  }
  if (app_Attributes().wantsMouseTracking) app_MouseMotion(xpos,ypos);
}
...
//  fprintf(stderr,"button %d, %d\n",button,action);
  ...
  switch (action) {
    ...
    case 1:
      ...
      mouseDownX = x;
      mouseDownY = y;
      // wait until there is a motion to inform app
      break;
  }
}
...
int main(int argc, char * argv[]) {
  SDL_Window * window = init(argc,argv);
  eventLoop(window);
