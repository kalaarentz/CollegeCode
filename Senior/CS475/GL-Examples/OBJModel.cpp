...
typedef std::map<std::tuple<int,int,int>,int> indexMap;
  ...
  while (1) {
    ...
    if (strcmp(tag,"v") == 0) {
      ...
      for (int i = 0; i < 3; i++) {
        ...
        fscanf(fd,"%31s",str);
        ParseFace(str,&v,&vt,&n);
        std::tuple<int,int,int> triple = std::make_tuple(v,vt,n);
        if (map.find(triple) == map.end()) {
          model.vrts.push_back(vrts[v]);
          model.nmls.push_back(nmls[n]);
          if (vt >= 0) model.uvs.push_back(uvs[vt]);
          map[triple] = model.vrts.size()-1;
        }
        face[i] = map[triple];
      }
      model.indx.push_back(Face3(face[0],face[1],face[2]));
      fscanf(fd,"%*[^\n]\n");
    }
    else {
      // read the remainder of the line
...
struct faceModel createUVs(struct faceModel model) {
  if (model.uvs.size() > 0) {
    fprintf(stderr,"model already has UVs\n");
    exit(1);
  }
